...
test('Testando cadastro de respostas', ()=> {
  modelo.cadastrar_pergunta('10 + 10 = ?');
  const perguntas = modelo.listar_perguntas();
  expect(perguntas.length).toBe(1);
  modelo.cadastrar_resposta(perguntas[0].id_pergunta, "20")
  modelo.cadastrar_resposta(perguntas[0].id_pergunta, "1")
  const respostas = modelo.get_respostas(perguntas[0].id_pergunta)
  expect(respostas[0].texto).toBe("20")
  expect(respostas[1].texto).toBe("1")
